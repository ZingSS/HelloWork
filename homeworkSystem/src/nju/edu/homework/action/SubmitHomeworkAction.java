...
public class SubmitHomeworkAction extends BaseAction{
	...
	public String toSubmitHomework() throws Exception {
		...
		if(homework.getState().equals(Common.PASS)){
			Grade studentGrade = gradeService.getGradeByUserAndHomework(user.getId(), homework.getId());
			
			homeworkVO = new StudentHomeworkVO(homework, ssgVo.isSubmit(), studentGrade.getGrade(), studentGrade.getComment());
			...
			for (String g : allGrades) {
				...
				if (g.contains(".")) {
					iGrades.add(Integer.parseInt(g.substring(0, g.indexOf('.'))));
				}
				else if(g.equals("无")){
					iGrades.add(0);
				}
